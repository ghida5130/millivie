...
export default function MainPageContentArea({ name, pathName }) {
    let [topRatedMovies, setTopRatedMovies] = useState();

    useEffect(() => {
        fetch(
            `https://api.themoviedb.org/3/movie/${pathName}?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&language=ko-KR`,
            {
                method: "GET",
            }
        )
